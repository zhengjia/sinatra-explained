...
# To get a hint, looking at the four line app, apparently the get method is avaiable in the context of the current app. So some methods are "imported" from sinatra to the current app. To get another hint, we create a file which only has one line: require 'sinatra', and run it. We can see a server starts!

# This brings out two areas we are going to cover: method lookup and and server startup in sinatra.

# First let's see where the get method is defined. get is a class method of Sinatra::Base. Since it's avaiable at the top level in the current app, it can either be a class method or an instance method defined at the top level main object. Let's see which case it is and how get becomes avaiable in the current app. (In case you don't know, methods defined on the top level becomes private instance methods of Object class; class methods defined on top level become singleton methods on the main object, which is an instance of Object.)
...
# In sinatra/lib/sintra/main.rb, it first calls require 'sinatra/base' to make sure Sinatra::Base is avaiable to it. We need to explain two methods: set and caller_files. set is a class method on Sinatra::Base and is also delegated from current app to Sinatra::Application.
...
# set method is interesting but a bit complicated. There are several forms to use the set method, which helps to explain it. First one is just set :some_option, "some_value". It will just be translated to set option, Proc.new{value}, which is exactly the second form. For the second form, it open the singleton class of Sinatra::Base and define three new methods there using the metadef private class method of Sinatra::Base. The three class methods are used as setter, getter, and question mark method. The getter is lazy evaludated, meaning content of the block is used as the method body and isn't called until the getter is called. The question mark method uses double bang to get the true/false value based on the truthness of the result of the getter method. The third form is that when the setter is already defined by previous calls to the set method, then when we use set method in the first form it doesn't go through the second form and defines the getter setter and question mark element again; instead it just used the already defined setter.
...
# As an example, if we have set :inline_templates, true, then we will have three class methods avaiable on Sinatra::Base: inline_templates which returns true, inline_templates? which returns true also, and inline_templates= which sets inline_templates to a new value. We will look at how the set method is typically used in later tutorials.
...
# Now we come to the option parsing. If the current app is supposed to be run and any arguments are passed in to run it, sinatra will set those settings based on the passed in arguments. You can refer to the full list of the avaiable settings in the "Available Settings" section in sinatra doc. The option parsing is pretty standard and I will just include a reference here http://ruby-doc.org/stdlib/libdoc/optparse/rdoc/classes/OptionParser.html
